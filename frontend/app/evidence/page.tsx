    ...
    <div className="space-y-6 animate-fade-in max-w-6xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-surface-border pb-4">
        <div>
          ...
          <p className="text-xs font-mono text-slate-400 mt-1">
            SHA-256 binary hash integrity verification and tamper detection
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-950/40 border border-blue-800/60 text-xs font-mono text-blue-300">
          ...
          <span>ALGORITHM: SHA-256 (256-bit Digest)</span>
        </div>
      </div>
      ...
      <div className="bg-surface-200 border border-surface-border rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="text-xs text-slate-300 leading-relaxed font-sans">
          <strong className="text-slate-100 font-mono">Chain of Custody Guarantee:</strong> Upon intrusion detection, IBVAP writes a snapshot image to disk and calculates its 64-character SHA-256 cryptographic hash. Clicking <strong>Verify Hash</strong> executes an on-demand re-hash of the image binary on disk to prove zero tampering.
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {evidenceList.map((item) => (
          <Card
            ...
            className="group hover:border-slate-600 transition-all cursor-pointer bg-surface-200"
          ...
          >
            ...
            <div className="space-y-2 text-xs font-mono">
              <div className="flex items-center justify-between text-slate-200 font-bold">
                ...
                <span className="text-slate-500 font-normal">{item.event_id}</span>
              </div>

              <div className="p-2 rounded bg-surface-300 border border-surface-border text-[11px] text-slate-400 break-all select-all">
                <span className="text-slate-500 block text-[9px]">SHA-256 DIGEST:</span>
